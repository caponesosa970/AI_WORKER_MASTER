...
## Validator Two: Control Flow and Scenarios

- Status: PASS.
- Checks: `47 passed / 0 failed`.
- If/Else/End If and For/End For stack: balanced, no underflow, final depth zero.
- Authorization order, one-node/two-attempt retry, forbidden paths: PASS.
- Exact 1/5/10/25/50 datasets: static PASS.
- Invalid-input, duplicate, blank, wrong-status, reply, unresolved, error-cell, source-order, and read-failure cases: static HOLD as required.

Implementation: independent PowerShell control-flow parser and scenario model.
