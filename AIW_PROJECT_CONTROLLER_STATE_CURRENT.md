...
Authorized release-finalization exception: Task 229 may also finalize only the exact persisted state `%AIWDeadArchiving = 1`, `%AIWDeadArchiveOwner` idle, and transaction phase `RELEASE_BOOLEAN_INTENT`. Before using explicit mode `RELEASE_BOOLEAN_FINALIZE`, it must prove complete nonliteral transaction identity, destination exactness and uniqueness or idempotency, source reread plus exact A:I clear/readback, matching persistent commit state, no remaining owner-controlled work, and immediate commit revalidation. Only then may Task 229 set `%AIWDeadArchiving = 0`. It may not clear an owner, select a row, start or resume transaction work, mutate a Sheet, accept any other phase, or mutate on missing or inconsistent proof. Any mismatch is HOLD without mutation.

Authorized integrated runtime scope is Tasks 18, 19, 199, plus only the minimum Task 229 recovery and `RELEASE_BOOLEAN_FINALIZE` entries. All other Task 229 actions and every unrelated runtime task, profile, scene, registry, lifecycle, Send, Archive, live, or production-activation behavior must remain unchanged.
